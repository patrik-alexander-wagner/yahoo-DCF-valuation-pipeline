...
def forecasting_income_statement(df, forecast_years=5):
    ...
    col_depreciation = 'Reconciled Depreciation'
...
def forecasting_balance_sheet(df, is_forecast_df):
    ...
    bs_items_sums = {
        ...
        col_other_current_liab: get_sum(col_current_liab) - get_sum(col_payables) - get_sum("Current Debt And Capital Lease Obligation"),
...
def forecast_cashflow(df, is_forecast_df, bs_forecast_df):
    ...
    depreciation = is_forecast_df['Reconciled Depreciation']
    ...
    cf_df['Change In Inventory'] = delta_inventory
    cf_df['Change In Accounts Receivable'] = delta_ar
    cf_df['Change In Accounts Payable'] = delta_ap
    
    # Calculate Investment in Other Current Assets/Liabilities
    cf_df['Investment in Other Assets'] = (bs_shifted.loc[target_index, "OtherCurrentAssets_agg"] - bs_combined.loc[target_index, "OtherCurrentAssets_agg"])
    
    cf_df['Investment in Other Liabilities'] = -(bs_shifted.loc[target_index, "OtherCurrentLiabilities_agg"] - bs_combined.loc[target_index, "OtherCurrentLiabilities_agg"])
    
    cf_df['Investment in Working Capital'] = delta_inventory + delta_ar + delta_ap + cf_df['Investment in Other Assets'] + cf_df['Investment in Other Liabilities']
    
    # Calculate Capex
    # Formula: -(PPE(t) - PPE(t-1) + Depreciation) -> (Old - New) - Depreciation
    cf_df['Capex'] = (bs_shifted.loc[target_index, "Net PPE"] - bs_combined.loc[target_index, "Net PPE"]) - depreciation
    
    # Calculate UFCF
    cf_df['UFCF'] = cf_df['Gross Cash Flow'] + cf_df['Investment in Working Capital'] + cf_df['Capex']
